...
router.get('/audit-logs', requireAuth, requirePermission('VIEW_AUDIT_LOGS'), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/reports', requireAuth, requirePermission('VIEW_AUDIT_LOGS'), async (req: AuthenticatedRequest, res: Response) => {
  try {
